...
/**
 * REST controller for managing {@link com.toolaoe.domain.Review}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ReviewResource {
        ...
        Optional<Review> result = reviewRepository
            ...
            .map(existingReview -> {
                if (review.getPoint() != null) {
                    existingReview.setPoint(review.getPoint());
                }
                if (review.getUserReviewId() != null) {
                    existingReview.setUserReviewId(review.getUserReviewId());
                }
                if (review.getType() != null) {
                    existingReview.setType(review.getType());
                }
                if (review.getStartDate() != null) {
                    existingReview.setStartDate(review.getStartDate());
                }
                if (review.getEndDate() != null) {
                    existingReview.setEndDate(review.getEndDate());
                }
     ...
     */
    @GetMapping("/reviews")
    public List<Review> getAllReviews(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
